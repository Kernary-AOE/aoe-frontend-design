...
## A11y
- Use ARIA grid pattern: container `role='grid'`, rows `role='row'`, cells `role='gridcell'`.
- Roving tabindex: only the focused day has `tabindex='0'`; all others `tabindex='-1'`.
- Update `aria-live='polite'` on the month title when navigating to a new month.
- Each day cell needs an accessible name including the full date: 'April 27, 2026, Monday'.
