...
## Behavior
- Grid uses `repeat(auto-fit, minmax(280px, 1fr))` so cards reflow naturally on resize.
- Cards have explicit `grid-column: span N` to claim wider widths for charts and tables.
- Each card is self-contained — moving it does not break the layout (no shared chrome).
...
## A11y
- Each card is an `<article>` with a heading; do not use generic divs.
- Charts must have `role='img'` and a descriptive `aria-label` summarizing the trend (or include a hidden table with the underlying data).
- Delta percentages must be reflected in an accessible name like 'Up 4.3 percent vs last week' — color/arrow alone is insufficient.
- Card menus need `aria-haspopup='menu'` and an accessible name like 'Card menu'.
- Skeletons during loading should have `aria-hidden='true'` and a separate `aria-live` region announcing 'Loading dashboard'.
