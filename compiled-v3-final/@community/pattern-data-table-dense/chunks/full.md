...
- Sticky header: `position: sticky; top: 0` on `<thead>` so it stays visible during scroll.
- Column header click toggles sort (asc -> desc -> none); reflect via `aria-sort` attribute.
- Click row to select; Shift+Click range-selects; Cmd/Ctrl+Click toggles individual selection.
...
## A11y
- Use real `<table>` with `<thead>`/`<tbody>`/`<th scope='col'>` — not a div soup.
- Sortable columns must reflect state via `aria-sort` (none / ascending / descending).
- Wrap scroll container in `role='region'` with an accessible name and `tabindex='0'` so keyboard users can scroll horizontally.
- Selection state on rows uses `aria-selected`; checkboxes must have accessible names (e.g., 'Select PRM-1024').
- Avoid color-only status badges — pair color with text or icon.
