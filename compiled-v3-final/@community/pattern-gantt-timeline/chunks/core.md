...
- Dependency arrows are decorative SVG with `aria-hidden='true'`; convey dependencies in the bar's accessible name or details panel instead.
- Use ARIA grid pattern with `role='grid'`/`role='row'`/`role='gridcell'` for keyboard table semantics.
- Provide a non-drag alternative (date inputs in a side panel) to reschedule tasks for keyboard and screen reader users.
